 ...
 * Chris: Test script to output URI counts
 * @params $DIR - the directory where the files should be stored
 *         $INPUT - the path to your wiki-dump
 *         $PIGNLPROC_JAR - the path to your PIGNLPROC jar   
 *         $LANG - the two-letter language code of your wiki-dump
 *         $MAX_NGRAM_LENGTH - the max length of Ngrams to consider as possible surface forms
 *         $MIN_SURFACE_FORM_LENGTH - the minimum length of a surface form in characters
 *
 */


SET job.name 'Wikipedia URI counts for $LANG'
...
STORE uriCounts INTO '$DIR/count_URIs.TSV' USING PigStorage();
